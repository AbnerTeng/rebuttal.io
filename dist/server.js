...
function checkInviteCode(code) {
    if (!INVITE_CODE || code.length !== INVITE_CODE.length)
        return false;
    try {
        return crypto_1.default.timingSafeEqual(Buffer.from(code), Buffer.from(INVITE_CODE));
    }
    catch {
        return false;
    }
...
app.post('/api/auth/signup', (req, res) => {
    ...
    if (!username || !inviteCode) {
        res.status(400).json({ error: 'Username and invitation code required' });
        return;
    }
